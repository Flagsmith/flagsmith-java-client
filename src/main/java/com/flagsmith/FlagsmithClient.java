...
public class FlagsmithClient {
   ...
   * 
   * <p>A trait with a value of null will remove the trait from the identity if it
   * exists.</p>
   * <p>To specify a transient trait, use the TraitConfig class with isTransient set
   * to true as the trait value.</p>
   * 
  ...
  /**
   * Get all the flags for the current environment for a given identity. Will also
   * upsert traits to the Flagsmith API for future evaluations, if isTransient set to false.
   * 
   * <p>A trait with a value of null will remove the trait from the identity if it
   * exists.</p>
   * <p>To specify a transient trait, use the TraitConfig class with isTransient set
  ...
  public List<Segment> getIdentitySegments(String identifier, Map<String, Object> traits)
    ...
    IdentityModel identityModel = getIdentityModel(
        ...
        (
          traits != null
          ? ModelUtils.getTraitModelsFromTraitMap(traits) : new ArrayList<TraitModel>()
        )
    );
