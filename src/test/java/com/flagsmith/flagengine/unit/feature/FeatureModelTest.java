...
public class FeatureModelTest {
    ...
    public void loadMultiVariateFeatureOptionWithoutId() throws Exception {
        String json = "{\"value\": 1}";
        MultivariateFeatureOptionModel variate = MultivariateFeatureOptionModel.load(JSONEncoder.getMapper().readTree(json), MultivariateFeatureOptionModel.class);
        Assert.assertNull(variate.getId());
    }

    public void loadMultiVariateFeatureStateWithoutId() throws Exception {
        String json = "{ \"multivariate_feature_option\":{\"value\": 1},\"percentage_allocation\": 10 }";
        MultivariateFeatureStateValueModel variate = MultivariateFeatureStateValueModel.load(JSONEncoder.getMapper().readTree(json), MultivariateFeatureStateValueModel.class);
        Assert.assertNull(variate.getId());
